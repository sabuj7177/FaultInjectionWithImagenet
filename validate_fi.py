...
def validate(net, model_path, image_producer, top_k=5):
    ...
    with tf.Session() as sesh:
        ...
        correct += np.sum(sesh.run(top_k_op, feed_dict={input_node: input_images, label_node: input_labels}))
        ...
        fi = ti.TensorFI(sesh, logLevel=10, name="convolutional", disableInjections=False)
        probs_after_injection = sesh.run(probs, feed_dict={input_node: input_images, label_node: input_labels})
        print("Probability after fault injection")
        print(probs_after_injection)
